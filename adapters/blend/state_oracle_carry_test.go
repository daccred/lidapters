...
func oraclePriceOnlyChange(t *testing.T, l oracleLayout, code string, rawValue int64) contractsv1.ContractDataChange {
	t.Helper()
	return stateChange(t, l.OracleContract, mustDecodeScVal(t, codeKeyXDR(t, l, code)), i128Val(rawValue))
}

func reservePrice(t *testing.T, state *contractsv1.LedgerState, l oracleLayout, code string) string {
	t.Helper()
...
func TestDecodeState_OraclePriceCarriesAcrossPriceOnlyLedgers(t *testing.T) {
	...
	stateN1, err := adapter.DecodeState(priorN, []contractsv1.ContractDataChange{
	...
	stateN2, err := adapter.DecodeState(stateN1, []contractsv1.ContractDataChange{
	...
	again, err := adapter.DecodeState(priorN, []contractsv1.ContractDataChange{
		oraclePriceOnlyChange(t, layout, "wBTC", newWBTC),
	}, layout.LedgerSeq+1)
	if err != nil {
...
func TestDecodeState_OraclePriceEvictionClearsReserve(t *testing.T) {
	...
	stateN1, err := adapter.DecodeState(priorN, []contractsv1.ContractDataChange{evict}, layout.LedgerSeq+1)
	if err != nil {
		t.Fatalf("decode ledger N+1: %v", err)
	}
